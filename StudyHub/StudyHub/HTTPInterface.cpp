...
QJsonArray HTTPInterface::getAllGroups(){
    ...
    // the HTTP request
    QNetworkRequest req( QUrl( QString("https://studyhub.herokuapp.com/studygroups") ) );
...
QJsonArray HTTPInterface::getAllUsers(){
    ...
    // the HTTP request
    QNetworkRequest req( QUrl( QString("https://studyhub.herokuapp.com/users") ) );
    QNetworkReply *reply = mgr.get(req);
    eventLoop.exec(); // blocks stack until "finished()" has been called
...
void HTTPInterface::getUserGroups(User current_user){
   QJsonArray json_array;
    ...
    // the HTTP request
    QNetworkRequest req( QUrl( QString("https://studyhub.herokuapp.com/mygroups") ) );
...
void HTTPInterface::postCreateGroup(QString department, QString class_number, QString date, QString time, QString description){
    ...
    QUrl myURL(QString("https://studyhub.herokuapp.com/studygroups"));
...
void HTTPInterface::postJoinGroup(QString group_id, int user_id){
    ...
    QUrl myURL(QString("https://studyhub.herokuapp.com/studygroups_users"));
    QNetworkRequest request(myURL);
...
void HTTPInterface::postCreateUser(QString email, QString password, QString firstname, QString lastname, QString username){
    ...
    QUrl myURL(QString("https://studyhub.herokuapp.com/users"));
    QNetworkRequest request(myURL);
    request.setHeader(QNetworkRequest::ContentTypeHeader,"application/x-www-form-urlencoded");
...
bool HTTPInterface::postLogin(QString email, QString password){
    ...
    QUrl myURL(QString("https://studyhub.herokuapp.com/login"));
    QNetworkRequest request(myURL);
...
QJsonObject HTTPInterface::getStudyGroup(QString group_id){
    ...
    QString url = "https://studyhub.herokuapp.com/studygroups/" + group_id;
    QUrl myURL(url);
    QNetworkRequest request(myURL);
...
void HTTPInterface::postLeaveGroup(QString group_id, int user_id){
    //QJsonArray json_array;
     ...
     // the HTTP request
     QNetworkRequest req( QUrl( QString("https://studyhub.herokuapp.com/seekdestroy") ) );
     req.setHeader(QNetworkRequest::ContentTypeHeader,"application/x-www-form-urlencoded");
     QNetworkReply *reply = mgr.post(req, headerData);
     eventLoop.exec(); // blocks stack until "finished()" has been called
...
void HTTPInterface::postCreateComment(QString group_id, QString username, QString comment_text){
    ...
    // the HTTP request
    QNetworkRequest req( QUrl( QString("https://studyhub.herokuapp.com/group_comments") ) );
    req.setHeader(QNetworkRequest::ContentTypeHeader,"application/x-www-form-urlencoded");
    QNetworkReply *reply = mgr.post(req, headerData);
    eventLoop.exec(); // blocks stack until "finished()" has been called
...
QJsonArray HTTPInterface::getGroupComments(QString group_id){
    QJsonArray json_array;
     ...
     // the HTTP request
     QNetworkRequest req( QUrl( QString("https://studyhub.herokuapp.com/groupchats") ) );
     req.setHeader(QNetworkRequest::ContentTypeHeader,"application/x-www-form-urlencoded");
     QNetworkReply *reply = mgr.post(req, headerData);
     eventLoop.exec(); // blocks stack until "finished()" has been called
...
void HTTPInterface::deleteUser(QString user_id){
        ...
        QString url = "https://studyhub.herokuapp.com/users/" + user_id;
        QUrl myURL(url);
        QNetworkRequest request(myURL);
...
void HTTPInterface::setAdminPrivilege(QString user_email){
    ...
    // the HTTP request
    QNetworkRequest req( QUrl( QString("https://studyhub.herokuapp.com/setAdminUser") ) );
    req.setHeader(QNetworkRequest::ContentTypeHeader,"application/x-www-form-urlencoded");
    QNetworkReply *reply = mgr.post(req, headerData);
    eventLoop.exec(); // blocks stack until "finished()" has been called
